...
# THINKTAX_BIN is set by install.sh, or override via environment
# Can be a path to cli.js (will use node) or 'thinktax' command in PATH
THINKTAX_BIN="${THINKTAX_BIN:-thinktax}"

COLOR_FG="${COLOR_FG:-0xffdbd6cc}"
COLOR_RED="${COLOR_RED:-0xffe7349c}"
COLOR_YELLOW="${COLOR_YELLOW:-0xfff2a633}"

# Run thinktax - use node if it's a .js file, otherwise run directly
PAYLOAD=""
if [[ "$THINKTAX_BIN" == *.js ]] && [ -f "$THINKTAX_BIN" ]; then
  PAYLOAD=$(node "$THINKTAX_BIN" sketchybar --format json 2>/dev/null)
elif command -v "$THINKTAX_BIN" &>/dev/null || [ -x "$THINKTAX_BIN" ]; then
  PAYLOAD=$("$THINKTAX_BIN" sketchybar --format json 2>/dev/null)
fi
